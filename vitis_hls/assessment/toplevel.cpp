...
void toplevel(uint32_t *ram, uint32_t *code) {
    ...
    *code = 0;
    ...
    dbg_printf("HW_META_WORDS: %lu, HW_WAYPOINT_WORDS: %lu, HW_WORLD_WORDS: %lu, HW_MAX_WORDS: %lu, HW_WORLD_OFFSET: %lu, HW_WORLD_PATH_OFFSET: %lu\r\n",
    		HW_META_WORDS, HW_WAYPOINT_WORDS, HW_WORLD_WORDS, HW_MAX_WORDS, HW_META_WORDS + HW_WAYPOINT_WORDS, HW_META_WORDS + HW_WAYPOINT_WORDS + HW_WORLD_WORDS);
    ...
    WAYPOINT_LOOP: for (uint8_t i = 0; i < waypoint_count - 1; ++i) {
        ...
        if (total_length < MAX_OUTPUT_PATH_LENGTH) {
            ...
            MAKE_PATH_LOOP: for (uint32_t i = 0; i <= length; ++i) {
            	...
            	local_ram[HW_META_WORDS + HW_WAYPOINT_WORDS + HW_WORLD_WORDS + path_index] = (((uint16_t)current.x) << 16) | ((uint16_t)current.y);

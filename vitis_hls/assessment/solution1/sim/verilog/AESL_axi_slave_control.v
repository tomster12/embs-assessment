...
//------------------------Parameter----------------------
`define TV_IN_ram "../tv/cdatafile/c.toplevel.autotvin_ram.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter ram_DEPTH = 1;
reg [31 : 0] ram_OPERATE_DEPTH = 0;
parameter ram_c_bitwidth = 64;
parameter ram_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [ram_c_bitwidth - 1 : 0] mem_ram [ram_DEPTH - 1 : 0];
reg ram_write_data_finish;
...
//write ram reg
reg [31 : 0] write_ram_count = 0;
reg write_ram_run_flag = 0;
reg write_one_ram_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & ram_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        ram_write_data_finish <= 0;
        write_ram_run_flag <= 0; 
        write_ram_count = 0;
        count_operate_depth_by_bitwidth_and_depth (ram_c_bitwidth, ram_DEPTH, ram_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_ram_run_flag <= 1; 
            write_ram_count = 0;
        end
        if (write_one_ram_data_done === 1) begin
            write_ram_count = write_ram_count + 1;
            if (write_ram_count == ram_OPERATE_DEPTH) begin
                write_ram_run_flag <= 0; 
                ram_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_ram
    integer write_ram_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] ram_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = ram_c_bitwidth;
    process_num = 0;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_ram_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write ram data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (ram_c_bitwidth < 32) begin
                        ram_data_tmp_reg = mem_ram[write_ram_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < ram_c_bitwidth) begin
                                ram_data_tmp_reg[j] = mem_ram[write_ram_count][i*32 + j];
                            end
                            else begin
                                ram_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    write (ram_data_in_addr + write_ram_count * four_byte_num * 4 + i * 4, ram_data_tmp_reg, write_ram_resp);
                end
                process_busy = 0;
                write_one_ram_data_done <= 1;
                @(posedge clk);
                write_one_ram_data_done <= 0;
            end   
            process_0_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_ram_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [ram_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (ram_c_bitwidth , factor);
  fp = $fopen(`TV_IN_ram ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_ram); 
      $finish; 
  end 
  read_token(fp, token); 
